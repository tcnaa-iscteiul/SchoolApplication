...
const StudentsSlice = createSlice({
    ...
    reducers: {
        ...
        updateUser(state, action) {
            const found = state.students.filter((student: IUser) => (student.email === action.payload.email));
            if (found[0]) {
                ...
                const index = state.students.findIndex((student: IUser) => (student.email === action.payload.email));
